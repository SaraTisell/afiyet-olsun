...
class BookingFormView(CreateView):
    ...
    def form_valid(self, form):
        ...
        # Count how many existing reservations there is
...
class UpdateReservation(UserPassesTestMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        existing_table_reservation = Reservation.objects.filter(
            reservation_date=reservation_date,
            reservation_time=reservation_time).exclude(
                reservation_id=reservation.reservation_id).count()

        # Checks that there is not 10 existing reservations
        # on the requested date and time
        if existing_table_reservation == 10:
            message = f"Sorry the requested {reservation_date} and"
            f"{reservation_time} is not available."
